function [ WaveR ] = performance_routine_P ( Wave )
%% parameters sweep
Pmax = Wave.Input.Pmax ;
P_range = Wave.Input.P_range ;
...
%     Check(x,y,z).Wave.I_c_ac = Wave.AC_Cap.I_c_ac ;
    ...
    % total
    WaveR.Converter.Efficiency(x,y,z) = WaveR.Semiconductor.Efficiency(x,y,z)*WaveR.Inductor.Efficiency(x,y,z)/100;
    WaveR.Converter.Losses(x,y,z) = WaveR.Semiconductor.Losses(x,y,z) + 3*WaveR.Inductor.Losses(x,y,z) ;

    Efficiency(x,y,z) = WaveR.Semiconductor.Efficiency(x,y,z)*WaveR.Inductor.Efficiency(x,y,z)/100;
...
if Wave.Input.plot_routine == 1
    ...
    subplot(1,2,1)
    plot(P_range/Pmax,WaveR.Converter.Efficiency,'b','LineWidth',lw)
    grid on
    ...
    subplot(1,2,2)
    hold on
    plot(P_range/Pmax,WaveR.Inductor.Losses,'b','LineWidth',lw)
    plot(P_range/Pmax,WaveR.Semiconductor.Losses,'r','LineWidth',lw)
    grid on
    xlabel('Output Power [p.u.]')
    ylabel('Losses [kW]')
