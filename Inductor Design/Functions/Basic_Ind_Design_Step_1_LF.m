function [ Core , Converter , pareto , Result] = Basic_Ind_Design_Step_1_LF ( Converter )
...
%     Converter.Spec.Inductor.Wire.CondMat= 'AL';
    ...
    Converter.Spec.Inductor.Ipp_Fs  = max(Converter.Wav.D_iLpha); %Peak-to-peak current at switching Freq
            ...
            if Core.Opt.Core_auto == 1
                for k_core=1:MaxCorePArt(k_core_Typ)
                    for k_stack=1:Core.Max_Stack
                        ...
                        [ Core ] = Powder_Core_design_LF( Core );
